...
export default function RateButton() {
    ...
    //repotへ遷移する際にログインしていれば、rateをsaveする。
    const toReport = () => {
        if (isLoggedIn) {
            navigate('/report');
        } else {
    ...
    return(
        <>
            ...
            <Card sx={{ minWidth: 275, maxWidth: 500, margin:"auto", padding:"20px" }}>
                ...
                <CardActions>
                    <Button size="small" onClick={toReport}><span style={{color:"#007bff",textDecoration:"underline",cursor:"pointer"}}>詳しく見る</span></Button>
                </CardActions>
            </Card>
            </Box>
            <Typography
